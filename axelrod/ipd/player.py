...
from axelrod.ipd.game import DefaultGame
from axelrod.ipd.history import History
from axelrod.ipd.random_ import random_flip
...
class IpdPlayer(BasePlayer):
    ...
    def __getstate__(self):
        """Used for pickling. Override if IpdPlayer contains unpickleable attributes."""
    ...
    def play(self, opponent, noise=0, strategy_holder=None):
        """This pits two players against each other, using the passed strategy
        holder, if provided."""
        if strategy_holder is None:
            strategy_holder = self
        return simultaneous_play(strategy_holder, opponent, noise)
