...
from axelrod.ipd.strategies._filters import *
...
class TestFilters(unittest.TestCase):
    ...
    def test_filtered_strategies(self):
        class StochasticTestStrategy(IpdPlayer):
            classifier = {
                "stochastic": True,
        ...
        class MemoryDepth2TestStrategy(IpdPlayer):
            classifier = {"stochastic": False, "memory_depth": 2, "makes_use_of": []}

        class UsesLengthTestStrategy(IpdPlayer):
            classifier = {
                "stochastic": True,
                "memory_depth": float("inf"),
                "makes_use_of": ["length"],
