...
class Attention(Player):
    """A player who uses an attention mechanism to analyse the game.

    Names:
    - Attention: Attention by Marc-Olivier Derouin
    """
    ...
    def __init__(self, model: Optional[PlayerModel] = None) -> None:
        super().__init__()
        if model is None:
...
class EvolvedAttention(Attention):
    ...
    def __init__(
        ...
        model = PlayerModel(PlayerConfig())
