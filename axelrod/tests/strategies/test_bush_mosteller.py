...
class TestBushMostellar(TestPlayer):
    ...
    def test_strategy(self):
        ...
        self.versus_test(axelrod.Defector(), expected_actions=actions,
                         attrs={"_stimulus": -0.20000000000000004,
                                "_c_prob": 0.375 , "_d_prob": 0.45},
                         seed=1)
        ...
        self.versus_test(axelrod.Cooperator(), expected_actions=actions,
                         attrs={"_stimulus": -1},
                         init_kwargs={"aspiration_level_divider": 0.1}, seed=1)

        #Ensures that the player will never play C or D if his probability is equal to 0
        actions = [(C, C)] * 100
        self.versus_test(axelrod.Cooperator(), expected_actions=actions,
                         init_kwargs={"d_prob": 0.0}, seed=1)

        actions = [(D, C)] * 100
        self.versus_test(axelrod.Cooperator(), expected_actions=actions,
                         init_kwargs={"c_prob": 0.0}, seed=1)
