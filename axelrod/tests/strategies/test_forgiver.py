...
class TestForgiver(TestPlayer):
    ...
    def test_cooperates_if_opponent_defections_is_ten_pct_and_defects_if_opponent_defections_gt_ten_pct(
        ...
        expected = [(C, D)] + [(D, C)] * 9
        self.versus_test(
            axelrod.MockPlayer(actions=final_action_lowers_defections_to_ten_percent),
    ...
    def test_never_defects_if_opponent_defections_le_ten_percent(self):
        ...
        expected = [(C, C)] * 9 + [(C, D)]
        self.versus_test(
            axelrod.MockPlayer(actions=defections_always_le_ten_percent),
            expected_actions=expected * 5,
        )
...
class TestForgivingTitForTat(TestPlayer):
    ...
    def test_strategy(self):
        ...
        self.versus_test(
            axelrod.Alternator(), expected_actions=[(C, C)] + [(C, D), (D, C)] * 5
        )
    ...
    def test_plays_tit_for_tat_while_defections_gt_ten_percent(self):
        ...
        self.versus_test(
            axelrod.MockPlayer(actions=before_tft), expected_actions=only_cooperates
        )

        now_alternator = before_tft + [D, C, D, C]
        now_tft = only_cooperates + [(C, D), (D, C), (C, D), (D, C)]
        self.versus_test(
            axelrod.MockPlayer(actions=now_alternator), expected_actions=now_tft
        )
