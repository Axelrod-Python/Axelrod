...
class TestCooperator(TestPlayer):
    ...
    def test_strategy(self):
        ...
        self.versus_test(opponent=axelrod.MockPlayer([C] * 11 + [D] * 4), expected_actions=actions)
              
        #Defect if in the last ten moves, Defections - Cooperations >= 3
        actions = [(C, D)] * 11 + [(D, C)] * 4
        self.versus_test(opponent=axelrod.MockPlayer([D] * 11 + [C] * 4), expected_actions=actions)
    
        #If neither of the above conditions are met, apply TitForTat 
        actions = [(C, D)] * 5 + [(C, C)] * 6 + [(C, D), (D, D), (D, D), (D, C), (C, C)]
        self.versus_test(opponent=axelrod.MockPlayer([D] * 5 + [C] * 6 + [D, D, D, C, C]), expected_actions=actions)
        
        actions = [(C, C)] * 5 + [(C, D)] * 6 + [(D, C), (C, C), (C, C), (C, D), (D, D)]
        self.versus_test(opponent=axelrod.MockPlayer([C] * 5 + [D] * 6 + [C, C, C, D, D]), expected_actions=actions)
