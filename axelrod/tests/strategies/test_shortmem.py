...
class TestCooperator(TestPlayer):
    ...
    def test_strategy(self):
        ...
        self.versus_test(
            opponent=axelrod.MockPlayer(actions=[C] * 11 + [D] * 4),
        ...
        self.versus_test(
            opponent=axelrod.MockPlayer(actions=[D] * 11 + [C] * 4),
            expected_actions=actions,
        )
        ...
        self.versus_test(
            opponent=axelrod.MockPlayer(actions=[D] * 5 + [C] * 6 + [D, D, D, C, C]),
            expected_actions=actions,
        )

        actions = [(C, C)] * 5 + [(C, D)] * 6 + [(D, C), (C, C), (C, C), (C, D), (D, D)]
        self.versus_test(
            opponent=axelrod.MockPlayer(actions=[C] * 5 + [D] * 6 + [C, C, C, D, D]),
            expected_actions=actions,
        )
