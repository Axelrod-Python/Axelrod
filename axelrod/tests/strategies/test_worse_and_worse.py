...
class TestWorseAndWorse(TestPlayer):
    ...
    def test_strategy(self):
        ...
        self.versus_test(axelrod.Cooperator(), expected_actions=actions, seed=8)
...
class TestWorseAndWorse2(TestPlayer):
    ...
    def test_strategy(self):
        ...
        self.versus_test(
            opponent=axelrod.MockPlayer(actions=[C, C, D, C]), expected_actions=actions
        )

        actions = [(C, C)] * 18 + [(C, D), (D, C)]
        self.versus_test(
            opponent=axelrod.MockPlayer(actions=[C] * 18 + [D, C]),
            expected_actions=actions,
        )
        ...
        self.versus_test(
            opponent=axelrod.MockPlayer(actions=[C] * 20 + [D, C, C, D]),
            expected_actions=actions,
            seed=8,
        )
        ...
        self.versus_test(
            opponent=axelrod.MockPlayer(actions=[C] * 20 + [D, C, C, C]),
            expected_actions=actions,
            seed=2,
        )
...
class TestWorseAndWorse3(TestPlayer):
    ...
    def test_strategy(self):
        ...
        self.versus_test(
            axelrod.MockPlayer(actions=[C, D, C, D, C]),
            expected_actions=actions,
            seed=8,
        )
