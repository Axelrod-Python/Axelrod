...
from axelrod.fingerprint import AshlockFingerprint, create_points, Point
...
class TestFingerprint(unittest.TestCase):
    ...
    def test_create_probes(self):
        ...
        probes = af.create_probes(progress_bar=False)
    ...
    def test_construct_tournament_elemets(self):
        af = AshlockFingerprint(self.strategy, self.probe)
        edges, tournament_players = af.construct_tournament_elements(0.5, progress_bar=False)
        self.assertEqual(edges, self.expected_edges)
    ...
    def test_progress_bar_fingerprint(self):
        af = AshlockFingerprint(self.strategy, self.probe)
        data = af.fingerprint(turns=10, repetitions=2, new_step=0.5,
                              progress_bar=True)
    ...
    def test_fingerprint_with_filename(self):
        ...
        af = AshlockFingerprint(self.strategy, self.probe)
        af.fingerprint(turns=1, repetitions=1, new_step=0.5, progress_bar=False,
                       filename=filename)
    ...
    def test_in_memory_fingerprint(self):
        af = AshlockFingerprint(self.strategy, self.probe)
        af.fingerprint(turns=10, repetitions=2, new_step=0.5, progress_bar=False,
                       in_memory=True)
    ...
    def test_serial_fingerprint(self):
        af = AshlockFingerprint(self.strategy, self.probe)
        data = af.fingerprint(turns=10, repetitions=2, new_step=0.5, progress_bar=False)
        edge_keys = sorted(list(af.interactions.keys()))
    ...
    def test_parallel_fingerprint(self):
        af = AshlockFingerprint(self.strategy, self.probe)
        af.fingerprint(turns=10, repetitions=2, new_step=0.5, processes=2,
                       progress_bar=False)
        edge_keys = sorted(list(af.interactions.keys()))
        coord_keys = sorted(list(af.data.keys()))
        self.assertEqual(af._step, 0.5)
    ...
    def test_plot(self):
        af = AshlockFingerprint(self.strategy, self.probe)
        af.fingerprint(turns=10, repetitions=2, new_step=0.25, progress_bar=False)
        p = af.plot()
    ...
    def test_wsls_fingerprint(self):
        ...
        data = af.fingerprint(turns=50, repetitions=2, new_step=0.25)
