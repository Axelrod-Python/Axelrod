...
from axelrod.fingerprint import (
    ...
    raise_error_for_mismatched_edges, raise_error_for_non_unit_square
)
...
class TestModuleFunctions(unittest.TestCase):
    ...
    def test_raise_error_for_mismatched_edges(self):
        interactions = {(0, 1): [[(C, C)]], (0, 2): [[(C, C)]]}
        too_long = {Point(1, 1): (0, 1), Point(2, 2): (0, 2),
                    Point(3, 3): (0, 3)}
        too_short = {Point(1, 1): (0, 1)}
        wrong_value = {Point(1, 1): (0, 1), Point(2, 2): (0, 3)}
        just_right = {Point(1, 1): (0, 1), Point(2, 2): (0, 2)}

        self.assertRaises(ValueError, raise_error_for_mismatched_edges,
                          too_long, interactions)
        self.assertRaises(ValueError, raise_error_for_mismatched_edges,
                          too_short, interactions)
        self.assertRaises(ValueError, raise_error_for_mismatched_edges,
                          wrong_value, interactions)
        self.assertIsNone(raise_error_for_mismatched_edges(just_right,
                                                           interactions))

    def test_raise_error_for_non_unit_square(self):
        unordered_square = [Point(1.0, 0.0), Point(0.0, 1.0),
                            Point(0.0, 0.0), Point(1.0, 1.0)]
        small_square = [Point(0.0, 0.0), Point(0.0, 0.5),
                        Point(0.5, 0.0), Point(0.5, 0.5)]
        big_square = [Point(0.0, 0.0), Point(0.0, 5.0),
                      Point(5.0, 0.0), Point(5.0, 5.0)]
        rectangle = [Point(0.0, 0.0), Point(0.0, 0.5), Point(0.0, 1.0),
                     Point(1.0, 0.0), Point(1.0, 0.5), Point(1.0, 1.0)]
...
class TestDataOrganizer(unittest.TestCase):
    def test_error_when_mismatched_keys_to_values(self):
        points_to_edges = {Point(1, 1): (0, 1)}
        interactions = {(0, 1): [[(C, C)]], (0, 2): [[C, C]]}
        self.assertRaises(ValueError, DataOrganizer, points_to_edges,
                          interactions)

    def test_error_when_points_not_square(self):
        points_to_edges = {Point(1, 1): (0, 1)}
        interactions = {(0, 1): [[(C, C)]]}
        self.assertRaises(ValueError, DataOrganizer, points_to_edges,
                          interactions)
