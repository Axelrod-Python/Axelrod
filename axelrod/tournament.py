...
class Tournament():  # pragma: no cover
    def __init__(self):
        raise DeprecationWarning("Please change path from axelrod.tournament.Tournament to axelrod.Tournament")


class IpdTournament(BaseTournament):
    def __init__(
        self,
    ...
    def play(
        ...
        if not build_results and not filename:
            warnings.warn(
                "IpdTournament results will not be accessible since "
            ...
            for interaction, results in interactions:
                ...
                for index, player_index in enumerate(index_pair):
                    ...
                    if results is not None:
                        ...
                        for state in states:
                            row.append(state_to_action_distributions[index][
                                           (state, C)])
                            row.append(state_to_action_distributions[index][
                                           (state, D)])
    ...
    def _run_parallel(self, processes: int = 2,
                      build_results: bool = True) -> bool:
        """
